...
test("credential lifecycle enforces issuer, reviewer, candidate and immutable audit permissions", async () => {
  ...
  try{
    ...
    await db.exec(`update public.profiles set full_name='Candidate Name' where id='${candidate}';insert into public.credential_issuers(user_id,role) values('${issuer}','ISSUER'),('${reviewer}','REVIEWER');`);
    ...
    const created=await db.query<{id:string;credential_id:string}>(`insert into public.credentials(owner_id,issuer_id,credential_type,category,title,issue_date) values($1,$2,'COURSE_CERTIFICATE','COURSE','Secure Systems',current_date) returning id,credential_id`,[candidate,issuerId]);
