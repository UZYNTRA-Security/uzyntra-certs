...
test("organizations enforce roles, verification, ownership, and cross-organization isolation", async () => {
  ...
  const verifiedPartner = "20000000-0000-4000-8000-000000000002";
  try {
    ...
    await db.exec(`insert into public.organizations(id,name,slug,organization_type,verified_status) values('${secondOrg}','Pending Partner','pending-partner','TRAINING_PROVIDER','PENDING'),('${verifiedPartner}','Verified Partner','verified-partner','TRAINING_PROVIDER','VERIFIED');insert into public.organization_members(organization_id,user_id,role) values('00000000-0000-4000-8000-000000000001','${admin}','ADMIN'),('00000000-0000-4000-8000-000000000001','${reviewer}','REVIEWER'),('00000000-0000-4000-8000-000000000001','${issuer}','ISSUER'),('00000000-0000-4000-8000-000000000001','${viewer}','VIEWER'),('${secondOrg}','${outsider}','ADMIN'),('${verifiedPartner}','${outsider}','ADMIN');update public.profiles set full_name='Candidate',username='candidate-one',visibility='public' where id='${candidate}';`);
    ...
    const partnerCredential = (await db.query<{ value: { id: string } }>("select public.create_credential_draft($1,$2,$3,'COURSE_CERTIFICATE','COURSE','Partner Credential',null,current_date,null,null) value", [outsider, verifiedPartner, candidate])).rows[0].value;
    await db.query("select public.transition_credential($1,$2,'submit',null)", [outsider, partnerCredential.id]);
    await db.query("select public.transition_credential($1,$2,'issue',null)", [outsider, partnerCredential.id]);
    ...
    assert.equal((await db.query("select id from public.credentials where id=$1", [partnerCredential.id])).rows.length, 1);
    assert.equal((await db.query("update public.credentials set title='Cross Org Edit' where id=$1 returning id", [draft.id])).rows.length, 0);
    assert.equal((await db.query("update public.profiles set full_name='Edited By Org' where id=$1 returning id", [candidate])).rows.length, 0);
    await db.query("select set_config('request.jwt.claim.sub',$1,false)", [issuer]);
    assert.equal((await db.query("select id from public.credentials where id=$1", [partnerCredential.id])).rows.length, 0);
    ...
    const candidateCredentials = await db.query<{ title: string; organization_id: string }>("select title,organization_id from public.credentials order by title");
    assert.deepEqual(candidateCredentials.rows.map((row) => row.title), ["Organization Credential", "Partner Credential"]);
    await db.exec("set role service_role");
    const publicProfile = (await db.query<{ value: { credentials: Array<{ title: string; issuer: string }> } }>("select public.get_public_profile('candidate-one') value")).rows[0].value;
    assert.deepEqual(publicProfile.credentials.map((credential) => `${credential.issuer}:${credential.title}`).sort(), ["UZYNTRA Security:Organization Credential", "Verified Partner:Partner Credential"]);
