...
export function authFixture({ confirmed = true, expired = false } = {}) {
  const jar = new Map<string, string>();
  const writes: { name: string; value: string; options: CookieOptions }[] = [];
  const cacheHeaders: Record<string, string> = {};
  const requests: { path: string; grant: string | null; body: Record<string, unknown> }[] = [];
  const user: User = {
  ...
  const fetcher: typeof fetch = async (input, init) => {
    ...
    requests.push({ path: url.pathname, grant: url.searchParams.get("grant_type"), body });
    ...
    if (url.pathname.endsWith("/signup")) return Response.json({ ...user, email_confirmed_at: undefined, identities: [] });
