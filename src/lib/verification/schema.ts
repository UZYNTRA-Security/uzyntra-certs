...
export const publicCredentialSchema = z.object({
  ...
  certificate_slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*-[a-z0-9]{5,16}$/),
  ...
  badges: z.array(z.object({ name: z.string().max(160), slug: z.string(), icon_url: z.string().regex(/^\/(?:badges\/[a-zA-Z0-9_-]+\.(png|svg|webp|jpg|jpeg)|api\/badge\/[a-f0-9-]{36})$/), category: z.string(), level: z.string().nullable() })),
});
