...
export async function proxy(request: NextRequest) {
  ...
  const response = path.startsWith("/auth/") || path === "/dashboard" || path.startsWith("/dashboard/") || path === "/issuer" || path.startsWith("/issuer/") || path === "/login" || path === "/register" || path === "/forgot-password" || path === "/reset-password"
  ...
  if (process.env.VERCEL_ENV !== "production" || ["/login", "/register", "/forgot-password", "/reset-password", "/verify", "/dashboard", "/issuer"].includes(path) || path.startsWith("/dashboard/") || path.startsWith("/issuer/") || path.startsWith("/auth/") || path.startsWith("/v/")) {
